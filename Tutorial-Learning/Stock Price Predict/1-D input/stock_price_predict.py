...
data=data[::-1]      
base_path = "/home/csrc/Desktop/stock/stock2" 

plt.figure()
plt.plot(data)
plt.show()
text_data_normalize=(data-np.mean(data))/np.std(data)
text_data_normalize=text_data_normalize[5111:6111]
...
for i in range(length+1,len(normalize_data)):
    z=normalize_data[i:i+time_step]
    test_z.append(z.tolist())
print np.shape(train_x)
test_start=train_x[-1]
print np.shape(test_start)
...
def lstm(batch):      
    ...
    init_state=multi_cell.zero_state(batch,dtype=tf.float32)
...
def prediction():
    ...
    with tf.Session() as sess:
        ...
        plt.plot(list(range(length+1, length +1+ len(predict))), predict, color='r')
        plt.plot(list(range(5111,6111)),text_data_normalize, color='y')
        plt.show()
